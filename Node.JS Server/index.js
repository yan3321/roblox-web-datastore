        ...
        if (request.method == 'POST') {
                ...
                if (bodyJson) {
                    ...
                    if (key && value) {
                        pool.getConnection(function(err, connection) {
                            ...
                            connection.query("REPLACE INTO `store` (`dataKey`, `value`) VALUES ('" + key + "', '" + valueJSON + "');", function(err, result) {
                                if (err) {
                                    connection.release();
                                    response.writeHead(400, {
                                        'Content-Type': 'text/html'
            ...
            if (key) {
                pool.getConnection(function(err, connection) {
                    ...
                    connection.query("SELECT * FROM `store` WHERE `dataKey` = '" + key + "';", function(err, result, fields) {
                        if (err) {
                            connection.release();
                            response.writeHead(400, {
                                'Content-Type': 'text/html'
